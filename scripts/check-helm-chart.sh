...
CHART_DIR="${ROOT_DIR}/deploy/helm/human-machine-runtime"
...
helm template human-machine-runtime "${CHART_DIR}" > "${RENDERED_DEFAULT}"
node "${LINT_SCRIPT}" "${RENDERED_DEFAULT}"

helm template human-machine-runtime "${CHART_DIR}" -f "${CHART_DIR}/values-dev.yaml" > "${RENDERED_DEV}"
node "${LINT_SCRIPT}" "${RENDERED_DEV}"

helm template human-machine-runtime "${CHART_DIR}" -f "${CHART_DIR}/values-prod.yaml" > "${RENDERED_PROD}"
