# Cockpit Client Suite — 架构设计

> 版本 1.0 | 2026-04-14 | MVP 阶段

## 1. 产品形态

Cockpit Client Suite 是面向终端用户（管理者）的 **React SPA**，实现"人机协同管理者模式"——用户做决策，AI 自主执行。
...
| **mock/** | MockCockpitDataSource, mockNotificationData, mockDecisionGoalData, MockAppTemplates, MockDocTemplates | Mock 数据工厂（演示模式） |
...
| 模块 | 文件 | 职责 |
|------|------|------|
| **stores/** | cockpitStore (+Types, +ConversationHelpers, +DiscussionActions) | Cockpit 主状态（对话/Agent/通知/决策/目标/看板） |
...
| 目录 | 文件数 | 职责 |
|------|-------|------|
| **features/cockpit/** | 37+14 | Cockpit 主界面 + 消息 blocks |
...
├── cockpitStore          # 主状态：对话、Agent、通知、决策、目标、看板
│   ├── cockpitTypes.ts           # 类型定义
│   ├── cockpitConversationHelpers.ts  # 纯函数辅助
│   └── cockpitDiscussionActions.ts    # 讨论处理
├── chatStore              # Matrix 对话
├── knowledgeStore         # 知识库
├── agentStore             # Agent 管理
...
- **数据源**：domain 定义实体结构，infrastructure 的 MockCockpitDataSource 提供数据
