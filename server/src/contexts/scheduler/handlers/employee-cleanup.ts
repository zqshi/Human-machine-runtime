 ...
 * 离职员工监测与清理作业
 *
 * 数据源：ClusterInstanceClient.listInstances（上游成员状态，含 lastActive/isActive/status）。
 * 判定：
...
function isManagerFlagged(inst: ClusterInstance): boolean {
...
function isInactive(
  ...
  days: number
): { resigned: boolean; lastActiveMs: number } {
...
export function registerEmployeeCleanup(
  ...
  clusterInstance: ClusterInstanceClient,
  ...
  handler.register('employee-cleanup', async (raw) => {
    ...
    try {
      const result = await clusterInstance.listInstances(1, 1000);
      instances = result?.items ?? [];
    } catch (e) {
      return {
        conclusion: `离职监测失败：无法读取集群实例列表（${(e as Error).message}）`,
    ...
    for (const inst of instances) {
      ...
      if (criteria === 'manager-flagged') {
        ...
        hit = r;
        if (r)
          reason = `最后活跃 ${new Date(lastActiveMs).toISOString().slice(0, 10)}（>${inactiveDays}天）`;
      }
      if (hit) {
        resigned.push({
