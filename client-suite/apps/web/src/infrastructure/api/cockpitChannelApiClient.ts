/**
 * Cockpit Channel API Client
 *
 * Wraps the /api/cockpit/channels/* endpoints for the Decision Console.
...
export const channelApi = {
  list(): Promise<ChannelListResult> {
    return request('/api/cockpit/channels');
  },

  getView(): Promise<AggregatedViewResult> {
    return request('/api/cockpit/channels/view');
  },
  ...
  getTimeline(options?: {
    ...
    const q = qs.toString();
    return request(`/api/cockpit/channels/timeline${q ? `?${q}` : ''}`);
  },
  ...
  dispatch(data: {
    ...
    return request('/api/cockpit/channels/dispatch', {
      method: 'POST',
  ...
  switchChannel(data: {
    ...
    return request('/api/cockpit/channels/switch', {
      method: 'POST',
      body: JSON.stringify(data),
    });
  },
