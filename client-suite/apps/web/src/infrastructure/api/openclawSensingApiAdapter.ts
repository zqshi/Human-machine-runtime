...
export async function fetchEmergentSignals(): Promise<EmergentSignalDTO[]> {
  const res = await request<{ items: EmergentSignalDTO[] }>('/api/openclaw/signals/emergent');
  return res.items;
}
...
export async function createEmergentSignal(
  ...
  return request('/api/openclaw/signals/emergent', {
    method: 'POST',
...
export async function updateEmergentSignal(
  ...
  return request(`/api/openclaw/signals/emergent/${encodeURIComponent(id)}`, {
    method: 'PATCH',
...
export async function fetchDetectedPatterns(): Promise<DetectedPatternDTO[]> {
  const res = await request<{ items: DetectedPatternDTO[] }>('/api/openclaw/patterns');
  return res.items;
}
...
export async function createDetectedPattern(
  ...
  return request('/api/openclaw/patterns', {
    method: 'POST',
...
export async function fetchKnowledgePatterns(filter?: {
  ...
  const res = await request<{ items: KnowledgePatternDTO[] }>(
    `/api/openclaw/knowledge/patterns${qs}`
  );
  return res.items;
}
...
export async function createKnowledgePattern(
  ...
  return request('/api/openclaw/knowledge/patterns', {
    method: 'POST',
    body: JSON.stringify(pattern),
  });
}
...
export async function fetchScorecards(filter?: {
  ...
  const res = await request<{ items: ScorecardDTO[] }>(`/api/openclaw/evaluation/scorecards${qs}`);
  return res.items;
}

export async function createScorecard(scorecard: Omit<ScorecardDTO, 'id'>): Promise<ScorecardDTO> {
  return request('/api/openclaw/evaluation/scorecards', {
    method: 'POST',
