...
import { CockpitApiAdapter } from '../cockpitApiAdapter';
...
describe('CockpitApiAdapter', () => {
  let adapter: CockpitApiAdapter;

  beforeEach(() => {
    adapter = new CockpitApiAdapter();
    mockFetch.mockReset();
  });
  ...
  describe('fetchDecisions', () => {
    ...
    it('passes status filter as query param', async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith(
        '/api/cockpit/decisions?status=pending',
  ...
  describe('updateTask', () => {
    it('sends PATCH and returns updated task', async () => {
      ...
      expect(url).toBe('/api/cockpit/tasks/task-1');
