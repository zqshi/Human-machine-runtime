...
export class CockpitApiAdapter implements ICockpitDataSource {
  async fetchDecisions(filter?: { status?: string }): Promise<DecisionRequest[]> {
    ...
    const res = await request<{ items: Record<string, unknown>[] }>(`/api/cockpit/decisions${qs}`);
  ...
  async respondDecision(
    ...
    const res = await request<{ decision: Record<string, unknown> }>(
      `/api/cockpit/decisions/${encodeURIComponent(id)}/respond`,
  ...
  async fetchTasks(): Promise<AgentTask[]> {
    const res = await request<{ items: Record<string, unknown>[] }>('/api/cockpit/tasks');
  ...
  async updateTask(
    ...
    const res = await request<Record<string, unknown>>(
      `/api/cockpit/tasks/${encodeURIComponent(id)}`,
  ...
  async createGoal(input: CreateGoalInput): Promise<UserGoal> {
    const res = await request<Record<string, unknown>>('/api/cockpit/goals', {
      method: 'POST',
  ...
  async decomposeGoal(id: string, apply = false): Promise<DecomposeResult> {
    const res = await request<Record<string, unknown>>(
      `/api/cockpit/goals/${encodeURIComponent(id)}/decompose`,
  ...
  async addCollaborator(goalId: string, userId: string): Promise<UserGoal> {
    const res = await request<{ goal: Record<string, unknown> }>(
      `/api/cockpit/goals/${encodeURIComponent(goalId)}/collaborators`,
  ...
  async removeCollaborator(goalId: string, userId: string): Promise<UserGoal> {
    const res = await request<{ goal: Record<string, unknown> }>(
      `/api/cockpit/goals/${encodeURIComponent(goalId)}/collaborators/${encodeURIComponent(userId)}`,
      { method: 'DELETE' }
    );
    return toUserGoal(res.goal);
  }
  ...
  async createWorkOrder(input: CreateWorkOrderInput): Promise<WorkOrder> {
    const res = await request<Record<string, unknown>>('/api/cockpit/workorders', {
      method: 'POST',
      body: JSON.stringify(input),
    });
  ...
  async respondWorkOrder(id: string, response: string): Promise<WorkOrder> {
    const res = await request<Record<string, unknown>>(
      `/api/cockpit/workorders/${encodeURIComponent(id)}/respond`,
  ...
  async fetchJudgmentRecords(filter?: { decisionId?: string }): Promise<JudgmentRecord[]> {
    ...
    const res = await request<{ items: Record<string, unknown>[] }>(
      `/api/cockpit/judgment-records${qs}`
    );
  ...
  subscribeEvents(handler: (event: CockpitEvent) => void): () => void {
    ...
    const connect = () => {
      if (stopped) return;
      es = new EventSource('/api/cockpit/events', { withCredentials: true });

      const eventTypes: CockpitEventType[] = [
        'connected',
        'decision:created',
        'decision:updated',
        'task:updated',
  ...
  async fetchBootstrap(): Promise<{
    ...
    return request('/api/cockpit/bootstrap');
  }
  ...
  async executeAgent(
    ...
    return request('/api/cockpit/agent/execute', {
      method: 'POST',
...
export const cockpitApiAdapter = new CockpitApiAdapter();
...
export async function createJudgmentRecord(
  ...
  return request('/api/cockpit/judgment-records', {
    method: 'POST',
...
export async function fetchSignals(filter?: {
  ...
  const res = await request<{ items: Record<string, unknown>[] }>(`/api/cockpit/signals${qs}`);
  return res.items;
}
