...
export function DiscussionInitPage() {
  ...
  const handleClose = () => {
    useCockpitStore.getState().setDiscussingNotificationId(null);
  };
              ...
              <div className="px-4 py-3">
                <div className="flex items-start gap-2">
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="text-[11px] font-medium text-slate-300 mb-0.5">
                ...
                <div className="space-y-1.5">
                  {timeline.map((item, i) => (
                    <div key={i} className="flex items-start gap-2">
                      <div
                        className={`w-1 h-1 rounded-full mt-1.5 shrink-0 ${item.isOwn ? 'bg-primary' : 'bg-slate-500'}`}
                      />
                      <div className="min-w-0 flex-1">
                        <span
                          className={`text-[10px] ${item.isOwn ? 'text-primary' : 'text-slate-500'}`}
                        >
                          {item.sender}
                        </span>
                  ...
                  {activeTasks.map((task) => {
                    ...
                    return (
                      <div key={task.id} className="flex items-center gap-2">
                        ...
                        {runningSub && (
                          <span className="text-[9px] text-slate-600 shrink-0 max-w-[120px] truncate">
                            {runningSub.name}
                          </span>
                        )}
                      </div>
