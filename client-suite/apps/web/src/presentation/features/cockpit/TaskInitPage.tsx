...
export function TaskInitPage({ onOpenDrawer }: TaskInitPageProps) {
  ...
  const handleClose = () => {
    useCockpitStore.getState().setDiscussingTaskId(null);
  };

  const handleExpandDetail = () => {
    useCockpitStore.getState().selectTask(task.id);
              ...
              <div className="flex-1 min-w-0 space-y-1.5">
                ...
                {task.subtasks.slice(0, 4).map((s) => (
                  <div key={s.id} className="flex items-center gap-1.5">
                    ...
                    />
                    <span
                      className={`text-[10px] truncate ${s.status === 'success' ? 'text-slate-400 line-through' : 'text-slate-300'}`}
                    >
                      {s.name}
                    </span>
                  </div>
