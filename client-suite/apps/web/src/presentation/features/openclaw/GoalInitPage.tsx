...
import type { DecomposeResult, TaskAssignment } from '../../../domain/agent/IOpenClawDataSource';
...
export function GoalInitPage({ onOpenDrawer }: GoalInitPageProps) {
  ...
  const _decisionRequests = useOpenClawStore((s) => s.decisionRequests);
  const _tasks = useOpenClawStore((s) => s.tasks);
  ...
  const handleDecompose = useCallback(
    async (apply: boolean) => {
      ...
      setDecomposing(true);
      try {
        const result = await useOpenClawStore.getState().decomposeGoal(discussingGoalId, apply);
