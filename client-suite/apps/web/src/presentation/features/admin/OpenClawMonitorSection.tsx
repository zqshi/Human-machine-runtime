...
import { openclawMonitorApi, analyticsApi } from '../../../application/services/adminApi';
...
export function OpenClawMonitorSection() {
  ...
  const fetchData = () => {
    Promise.all([
      openclawMonitorApi.costOverview(),
      openclawMonitorApi.sla(),
      openclawMonitorApi.alerts().catch(() => ({ alerts: [] })),
      openclawMonitorApi.performance(),
      analyticsApi.agentPerformance().catch(() => ({ topAgents: [] })),
      openclawMonitorApi.health().catch(() => ({ metrics: [] })),
    ])
      .then(([c, s, a, p, ap, h]) => {
        setCost(c);
        setSla(s);
