...
type LatencyData = { days: string[]; p50: number[]; p95: number[]; avg: number[] };
type SpendUser = { userId: string; count: number; totalTokens: number; estimatedCost: number };
...
export function OpenClawStatisticsSection() {
  ...
  const fetchData = useCallback(() => {
    Promise.all([
      openclawStatisticsApi.dau(days),
      openclawStatisticsApi.messages(days),
      openclawStatisticsApi.retention(days),
      openclawStatisticsApi.deptTokens(),
      openclawStatisticsApi.topUsers(20),
      openclawStatisticsApi.topUserSpend(20),
      openclawStatisticsApi.tokens(days),
      openclawStatisticsApi.latency(days).catch(() => ({ days: [], p50: [], p95: [], avg: [] })),
      openclawStatisticsApi.errorRate(days).catch(() => ({ days: [], values: [] })),
    ])
...
function DeptTable({ items }: { items: Record<string, unknown>[] }) {
...
function UsersTable({ items }: { items: Record<string, unknown>[] }) {
...
function SpendTable({ items }: { items: SpendUser[] }) {
...
function MiniDeptRank({ items }: { items: Record<string, unknown>[] }) {
  const typed = items as { name?: string; dept?: string; tokens: number }[];
...
function MiniUserRank({ items }: { items: Record<string, unknown>[] }) {
  const typed = items as { name: string; messages: number; tokens: number }[];
  if (!typed.length)
    return (
      <div className="flex items-center justify-center h-20 text-xs text-gray-400">暂无数据</div>
...
function MiniSpendRank({ items }: { items: SpendUser[] }) {
...
function modalConfig(panel: NonNullable<ModalPanel>) {
