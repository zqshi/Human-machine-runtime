 ...
 * 对话视图（SharedAgentChatView），复用 Cockpit 的对话收发能力（useAgentChat
 * + cockpitStore），不切 appMode/dock，不跳 Almighty 工作面板。
 *
 * TODO(runtime-port): 当前直接绑定 cockpitStore（cockpit 运行时）。
 * 后续「cockpit 可替换运行时 / Agent 定义与运行分离」任务应将此处的对话
...
export const sharedAgentChatService = {
   ...
   *
   * IM 模式未走 cockpitStore.initialize()（该初始化拉通知/事件等 OC 专属数据，
   * 仅在 appMode==='cockpit' 时由 WorkspacePage 触发）。此处先调幂等的
  ...
  open(agentId: string, name: string): void {
    const oc = useCockpitStore.getState();
    oc.initConversation();
   ...
   * activeInstanceId,让 useAgentChat chat 请求带真 instanceId → cockpit chat route
  ...
  openInstalledInstance(instanceId: string, name: string): void {
    ...
    useCockpitStore.getState().setActiveInstanceId(instanceId);
  },
