...
import { decisionActions } from './cockpitDecisionActions';
import { decisionTreeActions } from './cockpitDecisionTreeActions';
import { drawerActions } from './cockpitDrawerActions';
import { runtimeTaskActions } from './cockpitRuntimeTaskActions';
import { conversationActions } from './cockpitConversationActions';
import { artifactActions } from './cockpitArtifactActions';
import { sharedAgentActions } from './cockpitSharedAgentActions';
import {
  handleSetDiscussingNotificationId,
  handleSetDiscussingDecisionId,
  handleSetDiscussingTaskId,
  handleSetDiscussingGoalId,
} from './cockpitDiscussionActions';
import type { CockpitState } from './cockpitTypes';
import { goalActions, workOrderActions } from './cockpitGoalWorkOrderActions';
import { clearPersistedConversations } from './cockpitPersistence';
import { initializeCockpit } from './cockpitInitializer';
...
/**
 * useCockpitStore —— Cockpit 工作区全局 store（多聚合协调面）
 *
 ...
 * │ 4. 命令式跨模块访问：全仓 39 处 `useCockpitStore.getState()` 命令  │
 * │    式调用（20 个文件），多数跨聚合；硬拆需逐处重指向，回归面大。   │
 * │ 5. 缺安全网：cockpit store 无单测，仅靠 1 个 apiAdapter 测试 +     │
 * │    集成测试兜底，拆分无细粒度回归保护。                             │
 * │                                                                     │
 ...
 *
 * ┌─ 文件分工（已完成的模块化拆分）──────────────────────────────────┐
 * │ cockpitStore.ts                  本文件：state 定义 + reset/       │
 * │                                   initialize + 组装入口              │
 * │ cockpitTypes.ts                  CockpitState 接口 + StoreSet/Get │
 * │ cockpitDrawerActions.ts          Drawer 开关 + B 栏选择 +           │
 * │                                   rebuildAttentionItems 派生        │
 * │ cockpitConversationActions.ts    对话 CRUD / 会话生命周期 / 流式   │
 * │ cockpitRuntimeTaskActions.ts     Runtime/Task CRUD + 工作流干预    │
 * │ cockpitDecisionTreeActions.ts    决策树主动活动展开/后续动作       │
 * │ cockpitArtifactActions.ts        App/Document/Board 展示聚合 CRUD  │
 * │ cockpitSharedAgentActions.ts     共享 Agent 直聊 + 全局上下文重置  │
 * │ cockpitDecisionActions.ts        决策请求 CRUD + respondDecision   │
 * │                                   事务（含纠偏传播 + 判断记录落库） │
 * │ cockpitGoalWorkOrderActions.ts   目标 CRUD/拆解/下发 + 工单 CRUD   │
 * │ cockpitDiscussionActions.ts      setDiscussingNotificationId 等 4  │
 * │                                   个上下文切换处理器（生成首条分析  │
 * │                                   消息）                            │
 * │ cockpitConversationHelpers.ts    buildDeepDiscussionResponse 等     │
 * │                                   纯函数（CoT/blocks 构造，无依赖） │
 * │ cockpitSSEHandler.ts             SSE 订阅 → store 增量更新          │
 * │ cockpitInitializer.ts            bootstrap 拉取 + SSE 挂载 + 首编排 │
 * │ cockpitPersistence.ts            sessionStorage 对话持久化（纯函数）│
 * └────────────────────────────────────────────────────────────────────┘
 *
 * ┌─ 后续演进路线图（非阻塞，条件成熟后再做）────────────────────────┐
 * │ 阶段 1（低风险）：把 _sharedAgentMeta / composerPrefill / drawerWidth│
 * │   等纯 UI 状态抽到 useCockpitUIStore，组件改 import 即可，无跨聚  │
 * │   合事务。                                                          │
 ...
 * │ 前置条件：补齐 cockpit store 的单测（rebuildAttentionItems /      │
...
export const useCockpitStore = create<CockpitState>((set, get) => ({
  ...
  // ── Drawer + B-column selection + Attention rebuild (cockpitDrawerActions.ts) ──
  ...drawerActions(set, get),

  // ── Conversation management (cockpitConversationActions.ts) ──
  ...conversationActions(set, get),

  // ── Runtime/Task CRUD + workflow intervention (cockpitRuntimeTaskActions.ts) ──
  ...runtimeTaskActions(set, get),

  // ── Decision Tree actions (cockpitDecisionTreeActions.ts) ──
  ...decisionTreeActions(set, get),

  // ── Decision requests (cockpitDecisionActions.ts) ──
  ...decisionActions(set, get),

  // ── Goals + WorkOrders (cockpitGoalWorkOrderActions.ts) ──
  ...goalActions(set, get),
  ...workOrderActions(set, get),

  // ── App/Document/Board artifacts (cockpitArtifactActions.ts) ──
  ...artifactActions(set, get),

  // ── Shared Agent direct chat + global context reset (cockpitSharedAgentActions.ts) ──
  ...sharedAgentActions(set, get),

  // ── Discussion actions (delegated to cockpitDiscussionActions.ts) ──
  ...
  async initialize() {
    await initializeCockpit(set, get);
  },
