/**
 * cockpitTypes —— Cockpit 工作区 store 的类型契约
 *
 * CockpitState 是单一聚合接口（含 goal/decision/task/workOrder/conversation
 ...
 * 拆分约束与演进路线见 cockpitStore.ts 头注释。
 */
...
export interface CockpitState {
  ...
  boards: ProjectBoard[];
  drawerContent: CockpitDrawerContent | null;
  drawerWidth: number;
  ...
  openDrawer(content: CockpitDrawerContent): void;
  closeDrawer(): void;
  toggleDrawer(): void;
...
export type StoreSet = (
  partial: Partial<CockpitState> | ((state: CockpitState) => Partial<CockpitState>)
) => void;
export type StoreGet = () => CockpitState;
