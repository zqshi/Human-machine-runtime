/**
 * openclawGoalWorkOrderActions — 目标 + 工单 CRUD 动作
 *
 * 从 openclawStore 拆分，降低主文件行数。
 */
...
export function goalActions(set: StoreSet, get: StoreGet) {
  return {
    ...
    async createGoal(input: CreateGoalInput): Promise<UserGoal> {
      try {
        const goal = await openclawApiAdapter.createGoal(input);
    ...
    async decomposeGoal(goalId: string, apply = false): Promise<DecomposeResult> {
      ...
      try {
        const result = await openclawApiAdapter.decomposeGoal(goalId, apply);
...
export function workOrderActions(set: StoreSet, get: StoreGet) {
  return {
    async fetchWorkOrders() {
      try {
        const workOrders = await openclawApiAdapter.fetchWorkOrders({ status: 'pending' });
        set({ workOrders });
        get().rebuildAttentionItems();
      } catch {
        // silent
    ...
    async respondWorkOrder(id: string, response: string) {
      try {
        const updated = await openclawApiAdapter.respondWorkOrder(id, response);
    ...
    async createWorkOrder(input: CreateWorkOrderInput): Promise<WorkOrder> {
      const wo = await openclawApiAdapter.createWorkOrder(input);
