/**
 * cockpitSSEHandler —— 后端实时事件 → store 增量更新
 *
 * 将 SSE 推送的 CockpitEvent（task 进度/decision 变更/goal 更新/notification
...
import type { CockpitEvent } from '../../domain/agent/ICockpitDataSource';
import type { WorkOrderProps } from '../../domain/agent/WorkOrder';
import type { MessageBlock } from '../../domain/agent/MessageBlock';
import type { AppArtifact, DocumentArtifact } from './cockpitTypes';
import type { StoreApi } from 'zustand';
import type { CockpitState } from './cockpitTypes';

type StoreAccessors = {
  get: StoreApi<CockpitState>['getState'];
  set: StoreApi<CockpitState>['setState'];
};

export function createSSEHandler({ get, set }: StoreAccessors): () => void {
  return cockpitApiAdapter.subscribeEvents((event: CockpitEvent) => {
    switch (event.type) {
      case 'connected': {
        get().fetchWorkOrders();
        break;
...
// handleArtifactProgress / handleArtifactCompleted 已移除:
// v2.0 投产清障删 agent-executor 假进度后,artifact:progress/artifact:completed 事件不再产生
// (task/app/doc/board 意图识别仅广播 artifact:created;真实执行走 /agent/dispatch)。
// task 完成通知走真实 task:updated 事件(routes/cockpit/tasks.ts 发源)。
