/**
 * cockpitChannelStore — 决策控制台 Channel 聚合状态
 *
 * 管理多 channel 健康状态、统一时间线、跨 channel 消息分发。
 * 数据源：cockpitChannelApiClient
 */
...
import type {
  ...
  ConversationDTO,
  TimelineEntryDTO,
} from '../../infrastructure/api/cockpitChannelApiClient';
import { channelApi } from '../../infrastructure/api/cockpitChannelApiClient';

interface CockpitChannelState {
...
export const useCockpitChannelStore = create<CockpitChannelState>((set, get) => ({
  channels: [],
  availableChannels: [],
  conversations: [],
  timeline: [],
