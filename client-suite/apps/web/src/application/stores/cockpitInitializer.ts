/**
 * cockpitInitializer —— bootstrap 拉取 + SSE 挂载 + 首次编排
 *
 * initializeCockpit：从后端并行拉取 runtimes/tasks/decisions/goals/workOrders/
...
import { request as apiRequest } from '../../infrastructure/api/cockpitApiAdapter';
import { useNotificationStore } from './notificationStore';
import { useAgentStore } from './agentStore';
import { useJudgmentStore } from './judgmentStore';
import { useSignalStore } from './signalStore';
import { createSSEHandler } from './cockpitSSEHandler';
import type { CockpitState } from './cockpitTypes';

type SetFn = (
  partial: Partial<CockpitState> | ((s: CockpitState) => Partial<CockpitState>)
) => void;
type GetFn = () => CockpitState;
...
async function fetchCollaborationChainsFromAPI() {
  try {
    const res = await apiRequest<{ items?: Record<string, unknown>[] }>('/api/cockpit/sessions');
...
export async function initializeCockpit(set: SetFn, get: GetFn): Promise<void> {
  ...
  try {
    ...
    try {
      const bootstrap = await cockpitApiAdapter.fetchBootstrap();
    ...
    try {
      [tasks, decisionRequests, goals, workOrders] = await Promise.all([
        cockpitApiAdapter.fetchTasks(),
        cockpitApiAdapter.fetchDecisions(),
        cockpitApiAdapter.fetchGoals(),
        cockpitApiAdapter.fetchWorkOrders({ status: 'pending' }).catch(() => []),
      ]);
      cockpitApiAdapter
    ...
    console.error('[CockpitStore] initialize failed', err);
